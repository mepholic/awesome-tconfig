...
-- {{ XRandr
if conf.xrandr_cfg ~= false then
   for p = 1, table.getn(conf.xrandr_cfg) do
      local cfg_pair = conf.xrandr_cfg[p]
      
      if conf.xrandr_pri ~= false then
         if cfg_pair[1] == conf.xrandr_pri then
            cfg_pair[1] = cfg_pair[1] .." --primary" end
         if cfg_pair[2] == conf.xrandr_pri then
            cfg_pair[2] = cfg_pair[2] .." --primary" end
      end
...
elseif conf.xrandr_pri ~= false then
...
-- Lock screen
run_once("xautolock", "-detectsleep -time 5 -locker \"i3lock -t -i ".. conf.cfg_dir ..
            "res/EagleNebula.png\" -notify 30 -notifier \"notify-send -u critical "..
